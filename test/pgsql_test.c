/*
 * End to end test of the PostgreSQL module against a real server.
 *
 * Skips itself - exit code 77 - when the CMDBM_TEST_PGSQL_* variables do
 * not describe a server to talk to. test/docker/env.sh sets them for the
 * server of test/docker/compose.yml.
 */
#include "dbtest.h"

#define CONFFILE    "pgsql_test_conf.json"
...
static void DropSchema(CMDBM_Session *sess, CMUTIL_JsonObject *params)
{
    ...
    CMCall(sess, Execute, TESTDBID, "u.dropSequence", params);
}
...
int main(void)
{
    ...
    if (!DbTestWriteConf("PGSQL", "PGSQL", "data/pgsql_mapper.xml", CONFFILE))
        return TESTSKIP;
    ...
    CHECK(CMCall(sess, Execute, TESTDBID, "u.createTable", params) >= 0,
          "create table");
    CHECK(CMCall(sess, Execute, TESTDBID, "u.createSequence", params) >= 0,
          "create sequence");
    CHECK(CMCall(sess, Execute, TESTDBID, "u.createProcedure", params) >= 0,
          "create procedure");

    TESTCASE("insert with bind variables and selectKey(BEFORE)");
    ...
    CHECK(CMCall(params, Get, "userId") != NULL,
          "selectKey(BEFORE) put the sequence value into the parameters");
    ...
    if (row) {
        ...
        CHECK(score > 91.4 && score < 91.6, "double precision value");
        CHECK(CMCall(vact, GetValueType) == CMJsonValueBoolean,
              "boolean column");
        CHECK(CMCall(vact, GetBoolean) == CMTrue, "boolean value");
